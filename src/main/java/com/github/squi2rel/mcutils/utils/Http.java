...
//copied from github.com/Anuken/Arc
...
public class Http{
    protected static ExecutorService exec = new ThreadPoolExecutor(0, Integer.MAX_VALUE, 1, TimeUnit.MINUTES, new SynchronousQueue<>(), r -> {
    ...
    public static void get(String url, ConsT<HttpResponse, Exception> callback, Param<Throwable> error){
    ...
    public static class HttpResponse{
         ...
         */
        public Object2ObjectMap<String, ObjectSet<String>> getHeaders(){
            //convert between the struct types
            Object2ObjectMap<String, ObjectSet<String>> out = new Object2ObjectOpenHashMap<>();
            Map<String, List<String>> fields = connection.getHeaderFields();
            for(String key : fields.keySet()){
                if(key != null){
                    out.put(key, ObjectSet.of(fields.get(key).toArray(new String[0])));
                }
            }
            return out;
        }
    ...
    public static class HttpRequest{
        ...
        public String url;
        public Object2ObjectMap<String, String> headers = new Object2ObjectLinkedOpenHashMap<>();
        ...
        public Param<Throwable> errorHandler = e -> Bukkit.getLogger().log(Level.WARNING, e, () -> "HTTP ERROR!");
        ...
        public HttpRequest error(Param<Throwable> failed){
            errorHandler = failed;
            return this;
        }
        ...
        public void block(ConsT<HttpResponse, Exception> success){
            if(url == null){
                errorHandler.invoke(new RuntimeException("can't process a HTTP request without URL set"));
                return;
            }
                ...
                try{
                    ...
                    try{
                        ...
                        if(code >= 400){
                            ...
                            errorHandler.invoke(new HttpStatusException("HTTP request failed with error: " + code + " (" + status + ", URL = " + url + ")", status, new HttpResponse(connection)));
                        }else{
    ...
    public enum HttpStatus{
        ...
        private static Int2ObjectMap<HttpStatus> byCode;
        ...
        public static synchronized HttpStatus byCode(int code){
            if(byCode == null){
                byCode = new Int2ObjectOpenHashMap<>();
                for(HttpStatus status : HttpStatus.values()){
                    byCode.put(status.code, status);
                }
            }
            return byCode.getOrDefault(code, UNKNOWN_STATUS);
        }
    }
